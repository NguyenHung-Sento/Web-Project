    ...
    return (
        <div onClick={e => e.stopPropagation()} className='w-[400px] h-screen bg-white grid grid-rows-10 text-gray-700 p-6'>
            <header className='border-b font-bold text-2xl flex justify-between row-span-1 h-full items-center'>
                <span>Your Cart</span>
                <span onClick={() => dispacth(showCart())} className='p-2 cursor-pointer'><IoClose size={24} color='white' className='bg-red-500 rounded-full' /></span>
            </header>
            <section className='row-span-6 h-full flex flex-col gap-3 max-h-full overflow-y-auto py-3'>
                {!current?.cart && <span className='text-sm'>Chưa có sản phẩm nào</span>}
                {current?.cart && current?.cart?.map((el) => (
                    <div key={el._id} className='flex justify-between border rounded-lg items-center bg-gray-200'>
                        ...
                        <span onClick={() => updateCart({pid: el.product?.id})} className='p-2 rounded-full hover:bg-gray-700 cursor-pointer hover:text-white'><ImBin size={20} /></span>
                    </div>
                ))}
            </section>
            <div className='row-span-3 h-full'>
                <div className='flex items-center justify-between my-4 pt-4 border-t'>
                    ...
                    <span className='font-semibold text-xl text-cyan-600'>{`${formatMoney(current?.cart?.reduce((sum, el) => sum + Number(el.product?.price), 0))} VNĐ`}</span>
                </div>
                <span className='italic text-gray-700 text-sm'>Tới trang giỏ hàng để xem chi tiết</span>
                <Button handleOnClick={() => navigate(`/${path.DETAIL_CART}`)} fw style={'mt-8 bg-red-600 hover:bg-red-700'} children={'Trang Giỏ Hàng'} />
            </div>
        </div>
    )
}
